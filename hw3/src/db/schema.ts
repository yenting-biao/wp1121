...
export const tweetsTable = pgTable(
  "tweets",
  {
    ...
    startAt: timestamp("start_at").default(sql`now()`).notNull(),
    finishAt: timestamp("finish_at").default(sql`now()`).notNull(),
  },
  (table) => ({
    userHandleIndex: index("user_handle_index").on(table.userHandle),
    createdAtIndex: index("created_at_index").on(table.createdAt),
    startAtIndex: index("start_at_index").on(table.startAt),
    finishAtIndex: index("finish_at_index").on(table.finishAt),
